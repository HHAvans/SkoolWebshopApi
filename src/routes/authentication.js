...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        console.log(`EXECUTING QUERY ON DATABASE: SELECT UserId, Username, Password FROM [User] WHERE Email = @Email`);
        const request = pool.request();
        request.input('Email', sql.VarChar, email);
        const result = await request.query('SELECT UserId, Username, Password FROM [User] WHERE Email = @Email');
