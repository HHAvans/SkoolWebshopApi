...
async function sendWorkshopReminders() {
    try {
        ...
        const now = await pool.request().query('SELECT GETDATE() AS CurrentDateTime, DATEADD(HOUR, 24, GETDATE()) AS EndDateTime');
        ...
        if (workshopResult.recordset.length > 0) {
            ...
            const templateQuery = 'SELECT CONTENT FROM [EmailTemplate] WHERE Name = @templateName';
